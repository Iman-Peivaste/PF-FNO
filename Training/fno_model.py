...
#The architecht was orginaly develoed by :
#Li, Z.et all., “Fourier Neural Operator for Parametric Partial Differential Equations”, ICLR, 2021. doi:10.48550/arXiv.2010.08895.
class SpectralConv2dFast(nn.Module):
    ...
    def forward(self, x):
        ...
        out_ft = torch.zeros(
            batch_size, self.out_channels, x.size(-2), x.size(-1) // 2 + 1, dtype=torch.cfloat, device=x.device
        )
...
class FNO2D(nn.Module):
    def __init__(self, modes1, modes2, width, time_steps):
        ...
        self.fc0 = nn.Linear(self.time_steps + 2, width)
